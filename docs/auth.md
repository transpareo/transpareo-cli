...
`transpareo auth token [--scope a,b]` prints a fresh token, so a
script or a subagent can call the API for an hour without ever
seeing the secret:
...
`slow_down` (429) with `Retry-After` over the rate limit. Every
